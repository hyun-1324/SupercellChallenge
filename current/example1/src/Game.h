#pragma once

#include <SFML/Graphics/Drawable.hpp>
#include <SFML/Graphics/Font.hpp>
#include <SFML/Window/Keyboard.hpp>
#include <memory>
#include <vector>
#include "Constants.h"
#include "Weapon.h"

class Player;
class Game;
class GameInput;
class Vampire;
class BloodPools; // not implemented correctly yet
class Weapon;
class Arrow;

namespace sf { class Clock; }

class Game : public sf::Drawable
{
public:
    
    enum class State
    {
        WAITING,
        ACTIVE,
    };
    
    Game();
    ~Game();
    
    bool initialise();
    void initCursor();
    void update(float deltaTime);
    void resetLevel();
    void draw(sf::RenderTarget &target, sf::RenderStates states) const override;
    
    State getState() const { return m_state; }
    bool handleMouseClick(const sf::RenderWindow& window);

    void startGame();
    void setWeaponType(WeaponType Type);
    void onKeyPressed(sf::Keyboard::Key key);
    void onKeyReleased(sf::Keyboard::Key key);

    int getPoints() const;
    void setPoints(int points);

    Player* getPlayer() const;
    sf::Texture* getPlayerTexture() { return &m_playerTexture; }
    sf::Texture* getVampireTexture() { return &m_vampTexture; }
    sf::Texture* getBloodPoolTexture() { return &m_bloodPoolTexture; }

    void vampireSpawner(float deltaTime);

private:
    std::unique_ptr<Player> m_pPlayer;

    std::vector<std::unique_ptr<Vampire>> m_pVampires;
    std::vector<std::unique_ptr<BloodPools>> m_pBloodPools;

    State m_state;
    std::unique_ptr<sf::Clock> m_pClock;
    std::unique_ptr<GameInput> m_pGameInput;

    float m_vampireCooldown = 0.0f;
    float m_nextVampireCooldown = 2.0f;
    int m_spawnCount = 0;
    int points = 0;
    
    sf::Font m_font;
    sf::Texture m_vampTexture;
    sf::Texture m_playerTexture;
    sf::Texture m_bloodPoolTexture;

    WeaponType m_WeapType;
    sf::Text m_swordText;
    sf::Text m_bowText;

    //Cursor
    sf::Vector2i mousePosWindow;
    sf::Vector2f mousePosView;
    sf::Cursor cursor;
};
